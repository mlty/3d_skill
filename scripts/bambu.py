...
# ═══════════════════════════════════════════════════════════════════
# FTP Upload with Implicit FTPS + TLS Session Reuse
# ═══════════════════════════════════════════════════════════════════
...
class ImplicitFTP_TLS(FTP_TLS):
    ...
    def connect(self, host='', port=990, timeout=-999, source_address=None):
        ...
        self.sock = _sock.create_connection(
...
def ftp_upload_file(ip, access_code, local_path, remote_filename):
    """
    Upload file to Bambu Lab printer via implicit FTPS (port 990).
...
class LocalBackend:
    def __init__(self):
        ...
        ip = os.environ.get("BAMBU_IP", "172.20.10.2")
        serial = os.environ.get("BAMBU_SERIAL", "0948BB551400897")
        access_code = os.environ.get("BAMBU_ACCESS_CODE", "a805a1c7")
